...
STACK_NAME="${JIRA_TICKET:-audit}"

FUNCTIONS=(
    "${STACK_NAME}-audit-message-delimiter"
    "${STACK_NAME}-audit-firehose-reingest"
    "${STACK_NAME}-redrive-sns-dlq-events"
    "${STACK_NAME}-s3-copy-and-encrypt"
    "${STACK_NAME}-s3-key-rotation"
)
