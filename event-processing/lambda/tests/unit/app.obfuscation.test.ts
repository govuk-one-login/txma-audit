...
describe('Unit test for app handler', function () {
    ...
    beforeEach(() => {
        ...
        secretManagerMock
            .on(GetSecretValueCommand, {SecretId: 'no-data-secret'})
            .resolves({});

        secretManagerMock
            .on(GetSecretValueCommand, {SecretId: 'secret-binary'})
            .resolves({
            $metadata: {
                httpStatusCode: 200,
                requestId: '1',
        ...
        secretManagerMock
            .on(GetSecretValueCommand, {SecretId: 'secret-string'})
            .resolves({
            $metadata: {
                httpStatusCode: 200,
                requestId: '1',
        ...
        secretManagerMock
            .on(GetSecretValueCommand, {SecretId: 'unknown_arn'})
    ...
    it('has expected data in obfuscated result', async () => {
        ...
        const firehoseEvent = TestHelper.createFirehoseEventWithEncodedMessage(TestHelper.encodeAuditEvent(expectedData));
