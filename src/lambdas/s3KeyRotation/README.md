...
2. For each object:
   ...
   - Decrypt using Wrapper Key 1 (AWS Encryption SDK)
   - Re-encrypt with both Wrapper Key 1 + Wrapper Key 2
   - Write updated object back to S3
3. Return success/failure result for each task

## Environment Variables

- `WRAPPER_KEY_1_ID` - ARN of current KMS wrapper key
- `WRAPPER_KEY_2_ID` - ARN of new KMS wrapper key
