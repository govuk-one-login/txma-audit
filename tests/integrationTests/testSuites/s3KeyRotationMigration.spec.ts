...
import { createKmsClientProvider } from '../../support/utils/aws/kms/createKmsClientProvider'
import {
  PutObjectCommand,
  GetObjectCommand,
  DeleteObjectCommand,
...
const encryptWithSingleKey = async (
  ...
  const keyring = new KmsKeyringNode({
    generatorKeyId,
    clientProvider: createKmsClientProvider()
  })
  const { encrypt } = buildEncrypt()
  const { result } = (await encrypt(keyring, Readable.from(plaintext))) as {
    result: Buffer
  }
  return result
}
...
const decryptWithKeyring = async (
  ...
  plaintext: Buffer
  encryptedDataKeyCount: number
  encryptedDataKeyProviders: string[]
}> => {
  const keyring = new KmsKeyringNode({
    keyIds,
    clientProvider: createKmsClientProvider()
  ...
  const { plaintext, messageHeader } = (await decrypt(
    ...
    plaintext: Buffer
    messageHeader: {
      encryptedDataKeys: readonly { providerInfo: string }[]
    }
  }
  return {
    plaintext,
    encryptedDataKeyCount: messageHeader.encryptedDataKeys.length,
    encryptedDataKeyProviders: messageHeader.encryptedDataKeys.map(
      (k) => k.providerInfo
    )
  }
}
...
describe.todo('S3 Key Rotation Migration - Integration Tests', () => {
  ...
  describe('Happy Path - Successful Re-encryption', () => {
    ...
    it('should verify the bucket contains Data, WK1, and WK2 encrypted data keys after re-encryption', async () => {
      ...
      // Decrypt using WK1 (Generator Key) and inspect the message header
      // to verify both wrapper key ARNs are present as encrypted data keys.
      ...
      // Verify both WK1 and WK2 ARNs are present in the encrypted data keys
      expect(resultWithWk1.encryptedDataKeyProviders).toContain(generatorKeyId)
      expect(resultWithWk1.encryptedDataKeyProviders).toContain(backupKeyId)
    })
